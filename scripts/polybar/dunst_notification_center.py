...
for notif in notifications:
    ...
    label = Gtk.Label()
    # make new lines every 35 characters
    #notif["message"]["data"] = "\n".join([notif["message"]["data"][i:i+35] for i in range(0, len(notif["message"]["data"]), 35)])
    ...
    # set border color
    box.override_background_color(Gtk.StateFlags.NORMAL, Gdk.RGBA(0.21,0.23,0.25, 0.98))
...
grid.attach(box_utilities, 0, i, 1, 1)
i+=1
...
for util in utils:
    ...
    if ("fill" in utils[util]):
        print(subprocess.getoutput(utils[util]["fill"]))
...
#grid.attach(grid_utilities, 0, i+1, 1, 1)
...
scroll.get_style_context().add_provider(provider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION)
